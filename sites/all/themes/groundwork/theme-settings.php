...
include_once(drupal_get_path('theme', 'groundwork') .'/inc/template.theme-settings.inc');
...
*   A form array.
*/
function groundwork_settings($saved_settings, $subtheme_defaults = array()) {
  ...
  $defaults = groundwork_theme_get_default_settings('groundwork');
  ...
  $exportable_settings = array (
    ...
    'cleanup_node_id'                => $settings['cleanup_node_id'],
    'cleanup_node_classes_promote'   => $settings['cleanup_node_classes_promote'],
    'cleanup_node_classes_sticky'    => $settings['cleanup_node_classes_sticky'],
    'cleanup_node_classes_teaser'    => $settings['cleanup_node_classes_teaser'],
    'cleanup_node_classes_preview'   => $settings['cleanup_node_classes_preview'],
    'cleanup_node_classes_type'      => $settings['cleanup_node_classes_type'],
    'cleanup_node_classes_language'  => $settings['cleanup_node_classes_language'],
    'cleanup_comment_anonymous'         => $settings['cleanup_comment_anonymous'],
    'cleanup_comment_node_author'    => $settings['cleanup_comment_node_author'],
    ...
    'gw_admin_hide_help'                => $settings['gw_admin_hide_help'],
    ...
    if (module_exists('submitted_by') == FALSE) {
      ...
      foreach ((array('default' => 'Default') + node_get_types('names')) as $type => $name) {
        $form['node_type_specific']['submitted_by_container']['submitted_by'][$type] = array(
          ...
          '#description' => t('These settings allow you to modify the output of the Author and Date information shown on nodes. For more options you can install the <a href="!link">Submitted By</a> module.', array('!link' => 'http://drupal.org/project/submitted_by')),
          '#collapsible' => TRUE,
          '#collapsed' => TRUE,
        );
  ...
  $form['node_type_specific']['links_container'] = array(
    '#type' => 'fieldset',
    '#title' => t('Links'),
    '#description' => t('Links are the "links" displayed at the bottom of nodes.'),
    '#collapsible' => TRUE,
    '#collapsed' => TRUE,
  );
  ...
  $form['layout'] = array(
    '#type' => 'fieldset',
    '#title' => t('Style and layout settings'),
    '#collapsible' => TRUE,
    '#collapsed' => TRUE,
  );
  if ($settings['layout_enable_settings'] == 'on') {
    $image_path = drupal_get_path('theme', 'groundwork') .'/css/groundwork-images';
  ...
  // Hide help messages
  $form['admin_settings']['administration']['gw_admin_hide_help'] = array(
    '#type' => 'checkbox',
    '#title' => t('Hide help messages.'),
    '#default_value' => $settings['gw_admin_hide_help'],
  ...
  $form['themedev']['dev']['classses']['body_classes']['cleanup_classes_node_type'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .node-type-[type] classes.'),
  ...
  $form['themedev']['dev']['classses']['body_classes']['cleanup_classes_add_edit_delete'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print classes for node add, edit and delete pages (.node-[arg]).'),
  ...
  // Node classes
  $form['themedev']['dev']['classses']['node_classes'] = array(
    '#type' => 'fieldset',
    '#title' => t('Node Classes'),
    '#description' => t('Node classes apply to nodes. They print in the main wrapper DIV for all nodes) in node.tpl.php.'),
    '#collapsible' => TRUE,
    '#collapsed' => TRUE,
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_id'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print a unique ID for each node e.g. #node-1.'),
    '#default_value' => $settings['cleanup_node_id'],
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_sticky'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .node-sticky class for nodes set to sticky.'),
    '#default_value' => $settings['cleanup_node_classes_sticky'],
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_promote'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .node-promoted class for nodes promoted to front.'),
    '#default_value' => $settings['cleanup_node_classes_promote'],
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_teaser'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .node-teaser class on node teasers.'),
    '#default_value' => $settings['cleanup_node_classes_teaser'],
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_preview'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .node-preview class for node previews.'),
    '#default_value' => $settings['cleanup_node_classes_preview'],
  );
  $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_type'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .[content-type]-node classes.'),
    '#default_value' => $settings['cleanup_node_classes_type'],
  );
  if (function_exists('i18n_init')) {
    $form['themedev']['dev']['classses']['node_classes']['cleanup_node_classes_language'] = array(
      '#type' => 'checkbox',
      '#title' => t('Print .node-lang-[language] classes (requires i18n module)'),
      '#default_value' => $settings['cleanup_node_classes_language'],
    );
  }
  // Comment classes
  ...
  $form['themedev']['dev']['classses']['comment_classes']['comments']['cleanup_comment_node_author'] = array(
    '#type' => 'checkbox',
    '#title' => t('Print .comment-by-node-author for author comments.'),
    '#default_value' => $settings['cleanup_comment_node_author'],
  );
  ...
  if (module_exists('views')) {
    $form['themedev']['dev']['classses']['views_classes'] = array(
      ...
      '#description' => t('NOTE: If you are using custom Views templates you must use the template overrides that come with Groundwork to preserve this functality.'),
      '#collapsible' => TRUE,
      '#collapsed' => TRUE,
    );
  ...
  $form['themedev']['dev']['classses']['heading_classes'] = array(
    ...
    '#description' => t('Heading classes apply to node, block and comment titles (h2, h3 etc).'),
    '#collapsible' => TRUE,
    '#collapsed' => TRUE,
  );
  ...
  $form['themedev']['dev']['classses']['heading_classes']['cleanup_headings_namespaced_class'] = array(
    '#type' => 'checkbox',
    '#title' => t('Add a pseudo name spaced title class to headings, i.e. .node-title, .block-title, .comment-title.'),
